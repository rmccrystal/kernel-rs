...
void clean_piddb_cache() {
	...
	if (!PiDDBCacheTable)
	{
		...
		if (!PiDDBCacheTable)
		{
			...
			else
			{
				...
				if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
					...
					entry->DriverName = RTL_CONSTANT_STRING(L"bv74sa.sys"); //change driver name
				}
				...
				for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
				{
					...
					if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
						...
						cache_entry->DriverName = RTL_CONSTANT_STRING(L"bv74sa.sys");
					}
		...
		else
		{
			...
			/*capcom.sys(drvmap) : 0x57CD1415 iqvw64e.sys(kdmapper) : 0x5284EAC3*/
			if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
				entry->TimeDateStamp = 0x39EFC3; //change timestamp
				entry->DriverName = RTL_CONSTANT_STRING(L"js9fd.sys"); //change driver name
			}
			...
			for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
			{
				...
				if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
					cache_entry->TimeDateStamp = 0x39EFC3 + count;
					cache_entry->DriverName = RTL_CONSTANT_STRING(L"js9fd.sys");
				}
				//DbgPrint("cache_entry count: %lu name: %wZ \t\t stamp: %x\n", count, cache_entry->DriverName, cache_entry->TimeDateStamp);
			}
		}
	...
	else
	{
		...
		/*capcom.sys(drvmap) : 0x57CD1415 iqvw64e.sys(kdmapper) : 0x5284EAC3*/
		if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
			entry->TimeDateStamp = 0x41CEF1; //change timestamp
			entry->DriverName = RTL_CONSTANT_STRING(L"equrnds.sys"); //change driver name
		}
		...
		for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
		{
			...
			if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
				cache_entry->TimeDateStamp = 0x41CEF1 + count;
				cache_entry->DriverName = RTL_CONSTANT_STRING(L"equrnds.sys");
			}
			//DbgPrint("cache_entry count: %lu name: %wZ \t\t stamp: %x\n", count, cache_entry->DriverName, cache_entry->TimeDateStamp);
		}
	}
